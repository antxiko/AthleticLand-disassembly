...
_EMITTED = set()
# Etiqueta DATA_xxxx de cada bloque de datos declarado.
_DATANAMES = {}
...
def emit_data(data, org, a, b, dataranges, notes, names):
    ...
    for s, e, nm, d in hit:
        ...
        if e > cur:
            ...
            print(f"  aviso: el rango de datos {s:#06x}..{e:#06x} ({nm}) queda "
                  f"dentro de otro; su cabecera no se emite")
    if cur < b:
...
def emit_data_range(data, org, a, b, nm, desc, notes, names):
    ...
    # una DATA_xxxx, para que se vea donde empieza y se pueda apuntar a ella.
